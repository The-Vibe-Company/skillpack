-- Trigger proposal decision kind. Inert: nothing emits this kind yet. The generic
-- answer_decision path rejects it fail-closed until companion_api_answer_trigger_decision
...
ALTER TYPE public.companion_decision_request_kind ADD VALUE IF NOT EXISTS 'trigger_proposal';
--> statement-breakpoint

CREATE TABLE public.companion_triggers (
  ...
  CONSTRAINT companion_triggers_org_companion_id_uq UNIQUE (org_id, companion_id, id),
  CONSTRAINT companion_triggers_companion_fk
    FOREIGN KEY (org_id, companion_id)
    REFERENCES public.companions(org_id, id) ON DELETE CASCADE,
  CONSTRAINT companion_triggers_name_check
    CHECK (char_length(btrim(name)) BETWEEN 1 AND 80 AND name !~ E'[\n\r]'),
  CONSTRAINT companion_triggers_prompt_check
    CHECK (char_length(btrim(prompt)) BETWEEN 1 AND 16384),
  CONSTRAINT companion_triggers_provider_check
    CHECK (provider IN ('linear', 'github', 'custom')),
  CONSTRAINT companion_triggers_secret_check
    CHECK (secret ~ '^[0-9a-f]{32,128}$'),
  CONSTRAINT companion_triggers_error_check
    CHECK (
...
CREATE UNIQUE INDEX companion_triggers_name_uq
  ON public.companion_triggers (companion_id, lower(name));
--> statement-breakpoint

ALTER TABLE public.companion_triggers ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE public.companion_triggers FORCE ROW LEVEL SECURITY;
--> statement-breakpoint

CREATE POLICY "companion_triggers_function_owner_rls"
...
ALTER TABLE public.companion_turns
  ADD CONSTRAINT companion_turns_trigger_fk
  FOREIGN KEY (trigger_id) REFERENCES public.companion_triggers(id) ON DELETE SET NULL;
--> statement-breakpoint

ALTER TABLE public.companion_turns
  ADD CONSTRAINT companion_turns_trigger_origin_check
  CHECK (
    (trigger_id IS NULL OR trigger_name IS NOT NULL)
    AND (
      trigger_name IS NULL
      OR (
        char_length(trigger_name) BETWEEN 1 AND 80
        AND trigger_name !~ E'[\n\r]'
      )
    )
    AND NOT (routine_name IS NOT NULL AND trigger_name IS NOT NULL)
  );
--> statement-breakpoint

-- The same snapshot on the entry itself, for the same reason routine_name carries one:
-- companion_turns is private to the runtime function owner, so the conversation-list projection —
-- an ordinary API-role read of the transcript — cannot join it. Without this column the list would
-- preview a trigger prompt as something the Owner typed.
ALTER TABLE public.companion_transcript_entries
  ADD COLUMN trigger_name text;
--> statement-breakpoint

ALTER TABLE public.companion_transcript_entries
  ADD CONSTRAINT companion_transcript_entries_trigger_check
  CHECK (
    (
      trigger_name IS NULL
      OR (
        role::text = 'user'
        AND char_length(trigger_name) BETWEEN 1 AND 80
        AND trigger_name !~ E'[\n\r]'
      )
    )
    AND NOT (routine_name IS NOT NULL AND trigger_name IS NOT NULL)
...
ALTER TABLE public.companion_decision_deliveries
  ...
  CHECK (
    (
      proposal IS NULL
      AND request_kind::text NOT IN ('config_proposal', 'routine_proposal', 'trigger_proposal')
    )
    OR (
      request_kind::text IN ('config_proposal', 'routine_proposal', 'trigger_proposal')
  ...
  LOOP
    ...
    ELSIF v_event_type = 'decision' THEN
      ...
      IF v_event - ARRAY[
         ...
         OR (v_event ->> 'request_kind') NOT IN ('question', 'confirmation', 'config_proposal', 'routine_proposal', 'trigger_proposal')
         OR COALESCE(jsonb_typeof(v_event -> 'content'), 'missing') <> 'string'
         OR char_length(v_event ->> 'content') > 300
         ...
         OR (v_decision ->> 'kind') NOT IN ('shell', 'file', 'question', 'config', 'routine', 'trigger')
         ...
         OR ((v_event ->> 'request_kind' = 'trigger_proposal') IS DISTINCT FROM
             (v_decision ->> 'kind' = 'trigger'))
         OR (CASE WHEN v_event ->> 'request_kind' IN ('config_proposal', 'routine_proposal', 'trigger_proposal')
  ...
  LOOP
    ...
    ELSIF v_event_type = 'decision' THEN
      ...
      IF v_proposal = 'null'::jsonb THEN
        v_proposal := NULL;
      END IF;
      IF v_request_kind::text IN ('config_proposal', 'routine_proposal', 'trigger_proposal') THEN
...
BEGIN
  ...
  IF FOUND AND v_delivery.request_kind::text IN ('config_proposal', 'routine_proposal', 'trigger_proposal') THEN
...
DROP FUNCTION public.companion_api_enqueue_turn(
  uuid, uuid, uuid, text, public.companion_client_surface, jsonb, uuid, text
);
--> statement-breakpoint
...
CREATE FUNCTION public.companion_api_enqueue_turn(
  ...
  -- Trigger origin mirrors the routine pair; both are NULL for ordinary member sends and a turn
  -- never carries both.
    ...
    COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'trigger', CASE
          WHEN entry.role = 'user' THEN (
            SELECT CASE
              WHEN origin.trigger_name IS NULL THEN NULL
              ELSE jsonb_build_object(
...
CREATE FUNCTION public.companion_api_trigger_json(
  p_org_id uuid,
  p_companion_id uuid,
  p_trigger_id uuid,
...
CREATE FUNCTION public.companion_api_list_triggers(
  p_org_id uuid,
  p_companion_id uuid
)
RETURNS jsonb
...
AS $$
DECLARE
  -- The secret is the webhook credential. Viewer projections must never carry it, so the access
  -- level the read check already resolves decides whether the JSON includes it.
  v_access text := public.companion_api_require_access(p_org_id, p_companion_id, 'read');
  v_include_secret boolean := v_access IN ('owner', 'editor');
BEGIN
  RETURN COALESCE((
    SELECT jsonb_agg(
      public.companion_api_trigger_json(
        trigger_row.org_id, trigger_row.companion_id, trigger_row.id, v_include_secret
      )
      ORDER BY lower(trigger_row.name), trigger_row.id
    )
    FROM public.companion_triggers trigger_row
...
CREATE FUNCTION public.companion_api_rotate_trigger_secret(
  p_org_id uuid,
  p_companion_id uuid,
  p_trigger_id uuid,
  p_secret text
...
BEGIN
  PERFORM public.companion_api_require_access(p_org_id, p_companion_id, 'editor');
  IF p_trigger_id IS NULL
     OR p_secret IS NULL OR p_secret !~ '^[0-9a-f]{32,128}$' THEN
    RAISE EXCEPTION 'invalid Companion trigger secret' USING ERRCODE = '22023';
  END IF;
...
CREATE FUNCTION public.companion_api_delete_trigger(
  p_org_id uuid,
  p_companion_id uuid,
  p_trigger_id uuid
)
...
BEGIN
  ...
  IF p_trigger_id IS NULL THEN
    RAISE EXCEPTION 'invalid Companion trigger' USING ERRCODE = '22023';
  END IF;
  DELETE FROM public.companion_triggers trigger_row
  WHERE trigger_row.org_id = p_org_id
    AND trigger_row.companion_id = p_companion_id
    AND trigger_row.id = p_trigger_id;
...
-- Pre-session webhook path: this connection carries no tenant GUCs and no session, so there is no
-- access check to perform. The caller proves possession of the trigger UUID from the URL, and the
-- secret this row hands back is compared constant-time in TypeScript before anything fires. Row
-- lookup only; no mutation. row_security stays on: companion_triggers FORCEs RLS, and its
-- function-owner policy — not a tenant GUC — is what admits this SECURITY DEFINER owner.
CREATE FUNCTION public.companion_webhook_get_trigger(
  p_trigger_id uuid
)
RETURNS TABLE (
  org_id uuid,
...
SET row_security = on
AS $$
  SELECT trigger_row.org_id, trigger_row.companion_id, trigger_row.name, trigger_row.prompt,
    trigger_row.provider, trigger_row.secret, trigger_row.enabled
  FROM public.companion_triggers trigger_row
  WHERE trigger_row.id = p_trigger_id
$$;
--> statement-breakpoint

CREATE FUNCTION public.companion_api_fire_trigger(
  p_org_id uuid,
  p_trigger_id uuid,
  p_client_message_id uuid,
  p_content text
...
BEGIN
  ...
  SELECT trigger_row.* INTO v_trigger
  FROM public.companion_triggers trigger_row
  WHERE trigger_row.org_id = p_org_id AND trigger_row.id = p_trigger_id
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Companion trigger not found' USING ERRCODE = 'P0002';
  END IF;

  -- The replay check comes before every skip: a redelivered webhook whose first delivery already
  -- created the turn must resolve to that same durable turn, not to a throttle or pileup skip
  -- invented after the fact.
  SELECT EXISTS (
    SELECT 1
    FROM public.companion_turns existing_turn
    WHERE existing_turn.org_id = p_org_id
      AND existing_turn.companion_id = v_trigger.companion_id
      AND existing_turn.client_message_id = p_client_message_id
  ) INTO v_replay;
  ...
  IF NOT v_replay THEN
    ...
    SELECT EXISTS (
      ...
      WHERE queued.org_id = p_org_id
        AND queued.companion_id = v_trigger.companion_id
        AND queued.trigger_id = p_trigger_id
  ...
  -- Only a genuinely new fire advances last_fired_at. A provider redelivery replays an already
  -- accepted turn; letting it refresh the throttle window would silently swallow the next distinct
  -- event that arrives inside the extended window.
  UPDATE public.companion_triggers trigger_row
  SET last_fired_at = CASE WHEN v_replayed THEN trigger_row.last_fired_at
...
CREATE FUNCTION public.companion_api_fail_trigger_fire(
  p_org_id uuid,
  p_trigger_id uuid,
  p_error_code text,
...
DECLARE
  ...
  v_failures integer;
BEGIN
  IF p_org_id IS NULL OR p_trigger_id IS NULL
    ...
    RAISE EXCEPTION 'invalid Companion trigger failure' USING ERRCODE = '22023';
  END IF;

  UPDATE public.companion_triggers trigger_row
  SET consecutive_failures = trigger_row.consecutive_failures + 1,
  ...
  WHERE trigger_row.org_id = p_org_id
    AND trigger_row.id = p_trigger_id
  RETURNING trigger_row.consecutive_failures INTO v_failures;
  IF NOT FOUND THEN
    RETURN;
  END IF;
...
REVOKE ALL ON FUNCTION public.companion_api_trigger_json(uuid, uuid, uuid, boolean) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_list_triggers(uuid, uuid) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_create_trigger(uuid, uuid, uuid, text, text, text, text, boolean) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_update_trigger(uuid, uuid, uuid, text, text, text, boolean) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_rotate_trigger_secret(uuid, uuid, uuid, text) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_delete_trigger(uuid, uuid, uuid) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_webhook_get_trigger(uuid) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_fire_trigger(uuid, uuid, uuid, text) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_fail_trigger_fire(uuid, uuid, text, text) FROM PUBLIC;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.companion_api_enqueue_turn(uuid, uuid, uuid, text, public.companion_client_surface, jsonb, uuid, text, uuid, text) FROM PUBLIC;
--> statement-breakpoint

-- The split-role grant script runs before this migration (it is applied at the 0093 compatibility
-- checkpoint), so a post-cutover function must hand itself to the matching login. Triggers are
-- API-only: the webhook fires synchronously in the API request and no worker ever claims one.
DO $companion_triggers_acl$
DECLARE
...
BEGIN
  ...
  IF cardinality(v_grantees) = 1 THEN
    ...
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_list_triggers(uuid,uuid) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_create_trigger('
      || 'uuid,uuid,uuid,text,text,text,text,boolean) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_update_trigger('
      || 'uuid,uuid,uuid,text,text,text,boolean) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_rotate_trigger_secret('
      || 'uuid,uuid,uuid,text) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_delete_trigger(uuid,uuid,uuid) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_webhook_get_trigger(uuid) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_fire_trigger(uuid,uuid,uuid,text) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_fail_trigger_fire('
      || 'uuid,uuid,text,text) TO %I',
      v_role
    );
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION public.companion_api_enqueue_turn('
      || 'uuid,uuid,uuid,text,public.companion_client_surface,jsonb,uuid,text,uuid,text) TO %I',
      v_role
    );
