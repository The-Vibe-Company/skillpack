-- Expose the workspace retry counters to the claiming worker.
--
-- `companion_claim_project_workspaces` already increments `attempt` on every claim, and the claim
-- predicate keeps a `project_runtime_failed` workspace retryable while treating `available_at` as
-- the only backoff boundary. The worker could not honour that contract because the projection never
-- returned the counter, so it released every failed lease after one claim interval and retried a
-- permanently failing activation once per second indefinitely.
--
-- Adding columns to RETURNS TABLE changes the function result type, which CREATE OR REPLACE cannot
-- do, so the function is dropped and recreated with an identical body plus the two columns.
DROP FUNCTION IF EXISTS companion_claim_project_workspaces(text, integer, integer);--> statement-breakpoint
...
-- `apps/api/src/migrate.ts` re-applies runtime-role-grants.sql under the same advisory lock after
-- every migration, which restores the worker EXECUTE grant that DROP FUNCTION removed.
