-- Apply an approved trigger_proposal under the approver's authority. The generic
-- companion_api_answer_decision path stays fail-closed for this kind. Validation
-- failure rolls the whole transaction back so the delivery stays pending. The secret
-- is generated server-side in TypeScript and arrives here only to be stored; Pi's
-- proposal never contains one.

CREATE FUNCTION public.companion_api_answer_trigger_decision(
  p_org_id uuid,
  p_companion_id uuid,
  p_request_key text,
  p_action text,
...
BEGIN
  ...
  IF v_proposal IS NULL OR jsonb_typeof(v_proposal) <> 'object'
     ...
     OR EXISTS (
       ...
       WHERE patch_key NOT IN ('kind', 'name', 'prompt', 'provider')
     ) THEN
    RAISE EXCEPTION 'invalid Companion trigger proposal' USING ERRCODE = '22023';
  END IF;
...
REVOKE ALL ON FUNCTION public.companion_api_answer_trigger_decision(
  uuid, uuid, text, text, uuid, text
) FROM PUBLIC;
--> statement-breakpoint

DO $companion_api_trigger_decision_acl$
DECLARE
...
BEGIN
  ...
  EXECUTE format(
    'GRANT EXECUTE ON FUNCTION public.companion_api_answer_trigger_decision('
    || 'uuid,uuid,text,text,uuid,text) TO %I',
    v_role
  );
END
$companion_api_trigger_decision_acl$;
