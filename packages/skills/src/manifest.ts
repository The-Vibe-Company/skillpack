...
export async function prepareSkillDirForPublish(
  ...
  const normalizedManifest = fallbackSkillpackManifest({
    ...
    usage: companion.skillId && companion.instanceUrl ? { schemaVersion: 1, skillId: companion.skillId, version: companion.version, origin: new URL(companion.instanceUrl).origin } : existingManifest.metadata.usage,
  ...
  const skillpackManifest = fallbackSkillpackManifest({
    ...
    usage: normalizedManifest.metadata.usage,
