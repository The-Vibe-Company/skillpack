...
export async function reportSkillUsage(event: SkillUsageEvent, database: Db = db): Promise<void> {
  const parsed = skillUsageEventSchema.parse(event);
  await database.execute(sql`select public.skillpack_report_skill_usage(${JSON.stringify(parsed)}::jsonb)`);
}
...
export async function getSkillUsage(input: {
  ...
  const [result] = await input.database.execute<{ summary: SkillUsageSummary }>(sql`
    with events as materialized (
      select received_at, agent, environment, declared_user_id, declared_email, identity_source
