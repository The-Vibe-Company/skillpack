...
export function buildSkillpackSkillPrompts(version: string): LocalSkillPrompts {
  const auth = [
    ...
    "- Human setup: launch `skillpack auth login --api-url {base}` immediately when connection is missing or expired. Keep it running while the human approves the browser URL, then wait for exit and rerun `skillpack auth status --json`; continue the original task.",
    ...
    "- Never place an API key in a prompt, chat message, command argument, source file, or log. Do not stop after only presenting a login URL.",
  ].join("\n");
  ...
  const install = [
    ...
    runtime,
    "",
    `Use the requested or configured destination, then run: \`skillpack install skillpack --version ${version} --scope <scope> --tools <selected> --api-url {base} --json\`.`,
    "Reuse the existing install scope and tools when present. For a new install, use the user's stated destination; default to user scope and the configured tools when no destination was specified. Ask only when the destination cannot be inferred.",
    "Supported tools: Claude Code, Codex, OpenCode, Grok Bot, OpenClaw, or Hermes.",
    ...
    "Run `skillpack doctor --json`, then `skillpack setup --tools <selected>` when registration or hooks are missing; setup does not fetch packages. Rerun doctor to verify every target and prerequisite.",
  ...
  const update = [
    ...
    runtime,
    "",
    `Inspect the existing Skillpack lock entry to preserve its current scope and tools. Preview this management skill update with \`skillpack install skillpack --version ${version} --scope <existing-scope> --tools <existing-tools> --api-url {base} --dry-run --json\`, then run the same command without \`--dry-run\` when the request already authorizes the update.`,
    "If the lock has the historic thecompanion.sh origin or an intact untracked management copy, update the signed CLI first and retry the preview. The native installer migrates those known cases. Do not hand-edit the lock or reach for --force before inspecting a real customization conflict.",
    "Run `skillpack doctor --json` after installation. Check credentials and every selected target, then run `skillpack setup --tools <selected>` only if registration or hooks need repair and verify again.",
