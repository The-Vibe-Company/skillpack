...
describe("native Skillpack management prompts", () => {
  it("uses the native CLI and API-key flow for every generated prompt", () => {
    const prompts = buildSkillpackSkillPrompts("1.119.0");
    ...
    expect(text).toContain("skillpack install skillpack --version 1.119.0");
    ...
    expect(text).toContain("releases/download/runtime-v0.2.0/");
