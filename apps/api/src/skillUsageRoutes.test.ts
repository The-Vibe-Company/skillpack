...
const event = () => ({ event_id: randomUUID(), skill_id: randomUUID(), version: "1.0.0" });
function post(app: ReturnType<typeof createSkillUsageRoutes>, body: string) {
  return app.request("/v1/skill-usage", { method: "POST", headers: { "Content-Type": "application/json" }, body });
}
describe("anonymous activation reporting", () => {
  it("accepts anonymous and declared identities without credentials, with no receipt data", async () => {
