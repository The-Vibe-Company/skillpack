...
import { receiveSkillUsage } from "@skillpack/core";
...
export function createSkillUsageRoutes(input: { report?: typeof receiveSkillUsage; now?: () => number } = {}) {
  const app = new Hono();
  const report = input.report ?? receiveSkillUsage;
  const now = input.now ?? Date.now;
  let window = 0;
  let count = 0;
  app.post("/v1/skill-usage-events", async (c, next) => {
    ...
    try {
      const accepted = await report(parsed.data);
      if (accepted === false) {
        c.header("Retry-After", "60");
        return c.json({ error: "reporting capacity exceeded" }, 429);
      }
    } catch {
    ...
    return c.json({ event_id: parsed.data.event_id }, 202);
  });
  return app;
}
