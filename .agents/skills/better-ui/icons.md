...
Analyze composite icons part by part. A badge or slash overlay may keep its position even when the base glyph flips. For accessible names for icon-only buttons, consult the interaction and accessibility guidance in `design-frontend-dev`.
